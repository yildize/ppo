...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim:int, output_dim:int, hidden_dim:int, learn_std=False):
        super().__init__()
    ...
    def forward(self, obs):
        ...
        a1 = F.relu(self.linear_layer1(obs))
        a2 = F.relu(self.linear_layer2(a1))
...
class ActorCriticNetworks:
    ...
    def __init__(self, num_observations:int, num_actions:int, hidden_dim:int, multivariate_gauss_dist:MultivariateGaussianDist, learn_std:bool,  lr=3e-4):
        # Construct the actor and critic networks according to environment observation and action spaces:
        self.actor = MLP(input_dim=num_observations, output_dim=num_actions, hidden_dim=hidden_dim, learn_std=learn_std)
        self.critic = MLP(input_dim=num_observations, output_dim=1, hidden_dim=hidden_dim)
