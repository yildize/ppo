...
class InjectionSchedule:
    """ This class helps specifying the injection schedule, like from which step to
    ...
    class InjectionPeriod:
        """ This class specifies the details for a single injection period.
        A schedule can consists of one or more injection periods."""
    ...
    def add_a_period(self,  assistive_actor_type:AssitiveActors, step_start:int, step_end:int, assist_w_start:float=0.3, assist_w_end:float=0.0, noise_std:float=0.1, noise_update_freq:NoiseUpdateFreq=NoiseUpdateFreq.every_episode):
        """ Simple utilitiy method to add a new period to the schedule."""
        self.injection_periods.append(InjectionSchedule.InjectionPeriod(assistive_actor_type=assistive_actor_type,
                                                                        step_start=step_start, step_end=step_end,
                                                                        assist_w_start=assist_w_start, assist_w_end=assist_w_end,
                                                                        noise_std=noise_std, noise_update_freq=noise_update_freq))

    def get_current_injection(self, timestep)->Optional[InjectionPeriod]:
        """ Returns the currently active injection based on the current timestep"""
    ...
    def get_assistive_actor(self, period:InjectionPeriod):
        """ Creates the required assistive actor utilizing the factory."""
    ...
    def get_current_w(self, period:InjectionPeriod, current_timestep):
        """ Get the current assistant action weight from the provided period and timestep."""
    ...
    def __check_period_alone(self, period:InjectionPeriod):
        """ Check if period config is valid"""
